clear; clc;
addpath(genpath('rpnp'));
addpath dls;
addpath ASPnP;

display('$$$$ WARNING: EPnP is likely to fail in the planar case.$$$$');
display('$$$$ You may try the script several times to get the figures. $$$$');
display('$$$$ Or, simply remove EPnP from comparison. $$$$');
...
A= zeros(size(npts));
B= zeros(num,1);
name= {'SP+LHM', 'EPnP', 'RPnP', 'DLS',         'Reproj', 'ASPnP'};
f= {    @SP, @EPnP,  @RPnP, @dls_pnp, @PnP_Reproj_NLS_Matlab, @ASPnP};
marker= { 'x', 'o', 'd', '^','v', 's'};
color= {'r','g','b','m','c','k'};
markerfacecolor=  {'r','n','n','m','n','k'};
